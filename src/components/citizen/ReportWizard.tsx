...
export const ReportWizard: React.FC<ReportWizardProps> = ({ isOpen, onClose, onSubmitted }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-slate-950/80 backdrop-blur-md animate-fade-in overflow-y-auto">
      <div className="relative w-full max-w-xl bg-white rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[92vh] border border-slate-200">
        
        {/* Modal Top Header */}
        <div className="bg-gradient-to-r from-navy-950 via-navy-900 to-navy-950 text-white px-5 py-4 flex items-center justify-between border-b border-navy-800">
          <div>
        ...
        <div className="bg-slate-100 px-5 py-2.5 border-b border-slate-200 flex items-center justify-between text-xs font-bold text-slate-500">
          <div className={`flex items-center space-x-1.5 ${currentStep >= 1 ? 'text-emerald-700' : ''}`}>
            <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] ${currentStep >= 1 ? 'bg-emerald-600 text-white' : 'bg-slate-300 text-slate-700'}`}>1</span>
            <span>Details & Photos</span>
          </div>
          <ChevronRight className="w-4 h-4 text-slate-400" />
          <div className={`flex items-center space-x-1.5 ${currentStep >= 2 ? 'text-emerald-700' : ''}`}>
            <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] ${currentStep >= 2 ? 'bg-emerald-600 text-white' : 'bg-slate-300 text-slate-700'}`}>2</span>
            <span>AI Verification</span>
          </div>
          <ChevronRight className="w-4 h-4 text-slate-400" />
          <div className={`flex items-center space-x-1.5 ${currentStep >= 3 ? 'text-emerald-700' : ''}`}>
            <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] ${currentStep === 3 ? 'bg-emerald-600 text-white' : 'bg-slate-300 text-slate-700'}`}>3</span>
            <span>Publish</span>
          </div>
        </div>
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-50 border border-slate-300 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 rounded-2xl px-4 py-3 text-sm font-semibold text-slate-900 focus:outline-none"
                />
                ...
                <div className="mt-2 flex flex-wrap gap-1.5">
                  ...
                  {quickTitleSuggestions.slice(0, 3).map((sugg, i) => (
                    <button
                      ...
                      className="text-[11px] bg-slate-100 hover:bg-emerald-50 hover:text-emerald-800 text-slate-600 px-2 py-0.5 rounded-lg border border-slate-200 transition-colors truncate max-w-[280px]"
                    >
                      {sugg}
                    </button>
                  ))}
              ...
              <div>
                ...
                {category === 'Other' && (
                  <div className="mt-2.5 p-3 bg-amber-50 rounded-xl border border-amber-200 animate-fade-in">
                    ...
                    <input
                      ...
                      className="w-full bg-white border border-amber-300 rounded-lg px-3 py-2 text-xs font-semibold text-slate-900 focus:outline-none focus:ring-2 focus:ring-amber-400/40"
                    />
                  </div>
                )}
              </div>
              ...
              <div className="relative">
                <label className="block text-xs font-black text-slate-800 uppercase tracking-wider mb-1">
                  Full Address / Landmark <span className="text-rose-500">*</span>
                </label>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full bg-slate-50 border border-slate-300 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 rounded-2xl pl-10 pr-4 py-3 text-sm font-semibold text-slate-900 focus:outline-none"
                  />
              ...
              <div>
                ...
                <div className="relative">
                  <textarea
                    ...
                    className="w-full bg-slate-50 border border-slate-300 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 rounded-2xl p-3 text-sm text-slate-900 focus:outline-none"
                  />
                ...
                {voiceTranscription && (
                  <div className="mt-1.5 p-2 bg-emerald-50 border border-emerald-200 rounded-xl text-xs text-emerald-800 flex items-center space-x-2">
                    <span className="text-xs font-bold">🎤 Voice captured:</span>
                    <span className="italic truncate">{voiceTranscription}</span>
                  </div>
                )}
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                  {severities.map((sev) => {
                    ...
                    const colors = {
                      Low: 'hover:bg-emerald-50 text-emerald-800 border-emerald-300',
                      Medium: 'hover:bg-amber-50 text-amber-800 border-amber-300',
                      High: 'hover:bg-orange-50 text-orange-800 border-orange-300',
                      Critical: 'hover:bg-rose-50 text-rose-800 border-rose-300',
                    };
                    const selectedColors = {
                      Low: 'bg-emerald-600 text-white border-emerald-600',
                      Medium: 'bg-amber-500 text-slate-950 border-amber-500',
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <span className="text-[11px] text-slate-400">Up to 3 photos</span>
                </div>

                {/* Photo Gallery Grid */}
                <div className="grid grid-cols-3 gap-2 mb-2.5">
                  {photos.map((imgUrl, idx) => (
                    <div key={idx} className="relative rounded-2xl overflow-hidden border-2 border-emerald-500/60 aspect-video bg-slate-900 group">
                      ...
                      {photos.length > 1 && (
                        <button
                          ...
                          className="absolute top-1 right-1 bg-rose-600 text-white p-1 rounded-full shadow opacity-90 hover:opacity-100 transition-opacity"
                        >
                          <X className="w-3 h-3" />
                        </button>
                      )}
                  ...
                  {/* Add Photo Slot (if < 3) */}
                  {photos.length < 3 && (
                    <label className="border-2 border-dashed border-slate-300 hover:border-emerald-500 rounded-2xl flex flex-col items-center justify-center p-2 cursor-pointer bg-slate-50 hover:bg-emerald-50/50 transition-colors aspect-video">
                ...
                <div className="bg-slate-50 p-3 rounded-2xl border border-slate-200">
                  <p className="text-[10px] font-bold text-slate-500 uppercase mb-2">Or select a demo issue photo:</p>
                  <div className="flex flex-wrap gap-1.5">
                    {samplePresetImages.map((preset) => (
                      <button
                        ...
                        className="text-[11px] font-bold bg-white hover:bg-emerald-50 hover:text-emerald-900 text-slate-700 px-2.5 py-1 rounded-lg border border-slate-200 transition-all flex items-center space-x-1"
                      >
                        <span>📷</span>
              ...
              <div className="bg-slate-100 p-3.5 rounded-2xl border border-slate-200 flex items-start space-x-3">
                <input
                  type="checkbox"
                  id="reporterContact"
                ...
                />
                <label htmlFor="reporterContact" className="text-xs text-slate-700 cursor-pointer">
            ...
            <div className="space-y-5">
              {isAnalyzing ? (
                ...
                <div className="space-y-4">
                  {/* AI Results Card */}
                  <div className="bg-gradient-to-br from-emerald-50 via-teal-50 to-emerald-100 p-4 rounded-3xl border border-emerald-200 space-y-3">
                    <div className="flex items-center space-x-2 text-emerald-800">
                      <Sparkles className="w-5 h-5 text-emerald-600" />
                      <span className="font-black text-sm uppercase tracking-wider">AI Vision Classification</span>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2 pt-1">
                      <span className="bg-white px-2.5 py-1 rounded-xl text-xs font-bold text-emerald-800 border border-emerald-200">
                        Category: {category}
                      </span>
                      <span className="bg-white px-2.5 py-1 rounded-xl text-xs font-bold text-emerald-800 border border-emerald-200">
                        Severity: {severity}
                    ...
                    {aiTags.length > 0 && (
                      <div className="flex flex-wrap gap-1 pt-1">
                        {aiTags.map((tag, idx) => (
                          <span key={idx} className="text-[10px] bg-emerald-200/60 text-emerald-900 font-bold px-2 py-0.5 rounded-md">
                            #{tag}
                          </span>
                        ))}
                      </div>
                  ...
                  {nearbyDuplicate ? (
                    <div className="bg-amber-50 border-2 border-amber-300 rounded-3xl p-4 space-y-3 animate-fade-in">
                      <div className="flex items-start space-x-3">
                        <AlertTriangle className="w-6 h-6 text-amber-600 shrink-0 mt-0.5" />
                        <div>
                          <h4 className="text-sm font-black text-amber-950">{t.duplicateTitle}</h4>
                      ...
                      <div className="bg-white p-3 rounded-2xl border border-amber-200 flex items-center space-x-3">
                        <img src={nearbyDuplicate.duplicate.photoUrl} alt="Existing" className="w-12 h-12 rounded-xl object-cover" />
                        <div className="text-xs min-w-0">
                          <p className="font-bold text-slate-900 truncate">{nearbyDuplicate.duplicate.title}</p>
                          <p className="text-slate-500 text-[11px]">{nearbyDuplicate.duplicate.location.address}</p>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-2 pt-1">
                        <button
                          type="button"
                          onClick={handleMerge}
                          className="bg-amber-500 hover:bg-amber-400 text-slate-950 font-black text-xs py-2.5 px-3 rounded-xl shadow transition-all flex items-center justify-center space-x-1"
                        >
                        ...
                        <button
                          ...
                          className="bg-white hover:bg-slate-50 text-slate-700 border border-slate-300 font-bold text-xs py-2.5 px-3 rounded-xl transition-all"
                        >
              ...
              <div className="bg-slate-50 p-4 rounded-3xl border border-slate-200 space-y-3">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <h3 className="text-sm sm:text-base font-black text-slate-900 mt-1">{title || 'Civic Issue Listing'}</h3>
                  </div>
                  <span className={`text-[10px] font-black px-2 py-0.5 rounded-full ${
                    severity === 'Critical' ? 'bg-rose-100 text-rose-800' : 'bg-amber-100 text-amber-800'
                  }`}>
                    {severity} Priority
                  </span>
                </div>
                ...
                <p className="text-xs text-slate-700 bg-white p-3 rounded-2xl border border-slate-100">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {photos.map((p, idx) => (
                    <img key={idx} src={p} alt={`Photo ${idx + 1}`} className="w-full h-20 rounded-xl object-cover border border-slate-200" />
                  ))}
                </div>

                <div className="text-[11px] text-slate-500 flex items-center justify-between pt-1 border-t border-slate-200">
                  <span>Reporter: <strong>{currentUser.name}</strong></span>
                  <span>Target SLA: <strong>{severity === 'Critical' ? '4 Hours' : '24 Hours'}</strong></span>
                </div>
              </div>

              <div className="bg-amber-400/15 border border-amber-400/30 p-3 rounded-2xl flex items-center space-x-2 text-xs text-amber-900 font-semibold">
        ...
        <div className="bg-slate-50 px-5 py-4 border-t border-slate-200 flex items-center justify-between">
          {currentStep > 1 ? (
            <button
              ...
              className="px-4 py-2.5 rounded-xl border border-slate-300 text-slate-700 hover:bg-slate-100 font-bold text-xs flex items-center space-x-1 transition-colors"
            >
            ...
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2.5 rounded-xl text-slate-500 hover:text-slate-800 font-bold text-xs transition-colors"
            >
              Cancel
            </button>
          )}
          ...
          {currentStep === 1 && (
            <button
              ...
              className="bg-emerald-600 hover:bg-emerald-500 text-white font-black text-xs py-3 px-6 rounded-2xl shadow-lg shadow-emerald-600/30 flex items-center space-x-2 transition-all active:scale-95"
            >
          ...
          {currentStep === 2 && (
            <button
              ...
              className="bg-emerald-600 hover:bg-emerald-500 text-white font-black text-xs py-3 px-6 rounded-2xl shadow-lg shadow-emerald-600/30 flex items-center space-x-2 transition-all active:scale-95 disabled:opacity-50"
            >
          ...
          {currentStep === 3 && (
            <button
              ...
              className="bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-black text-xs py-3.5 px-6 rounded-2xl shadow-xl shadow-emerald-600/30 flex items-center space-x-2 transition-all active:scale-95"
            >
